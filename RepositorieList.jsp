...
<div id=Search_Middle align="right">
			...
			<tr>
				...
				if(totalCount > 0) {
					...
					if(pageNumTemp > 1){
						...
						sb.append("<a href=\"").append("RepositorieList.jsp?pageNum=1&amp;searchType="+searchType+"&amp;searchText="+searchText );
						sb.append("\" title=\"<<\"><<</a>&nbsp;");
					}
					if (isPrev) {
						int goPrevPage = startPage - pagePerBlock;			
						sb.append("&nbsp;&nbsp;<a href=\"").append("RepositorieList.jsp?pageNum="+goPrevPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						sb.append("\" title=\"<\"><</a>");
					} else {}
					...
					for (int i = startPage; i <= endPage; i++) {
						if (i == pageNumTemp) {
							...
							sb.append("<a href=\"").append("RepositorieList.jsp?pageNum="+i+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
					...
					if (isNext) {
						...
						sb.append("<a href=\"").append("RepositorieList.jsp?pageNum="+goNextPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						sb.append("\" title=\">\">></a>");
					} else {}
					if(totalNumOfPage > pageNumTemp){
						sb.append("&nbsp;&nbsp;<a href=\"").append("RepositorieList.jsp?pageNum="+totalNumOfPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						sb.append("\" title=\">>\">>></a>");
					}
	...
	<%
		...
		if (!"".equals(whereSQL)) {
			...
			if ("ALL".equals(searchType)) {
				...
				pstmt.setString(2, searchTextUTF8);
				pstmt.setString(3, searchTextUTF8);
				pstmt.setInt(4, listCount_Log * (pageNumTemp-1));
				pstmt.setInt(5, listCount_Log);			
			} else {
				pstmt.setString(1, searchTextUTF8);
				pstmt.setInt(2, listCount_Log * (pageNumTemp-1));
				pstmt.setInt(3, listCount_Log);			
			}
		} else {	
			pstmt.setInt(1, listCount_Log * (pageNumTemp-1));
			pstmt.setInt(2, listCount_Log);
		}
	...
	<div>
		...
		<br/><br/>
			<table cellspacing="0" cellpadding="25" summary="로그목록">
		...
		<tbody>
			...
			<tr>
				<td id=line>
				<%=rs.getString("WRITER") %> 가 <a class="links" href="boardView.jsp?num=<%=rs.getInt("NUM")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("SUBJECT") %></a> 수정을 요청 했습니다.		
				<br/><br/><%=rs.getString("MOD_DATE").substring(0, 16) %>
				</td>
		...
		<tfoot>
			...
			<% 
				...
				if(totalCount > 0) {
					int totalNumOfPage = (totalCount % listCount == 0) ? 
							totalCount / listCount_Log :
							totalCount / listCount_Log + 1;
					
					int totalNumOfBlock = (totalNumOfPage % pagePerBlock == 0) ?
							totalNumOfPage / pagePerBlock_Log :
							totalNumOfPage / pagePerBlock_Log + 1;
					
					int currentBlock = (pageNumTemp % pagePerBlock == 0) ? 
							pageNumTemp / pagePerBlock_Log :
							pageNumTemp / pagePerBlock_Log + 1;
					
					int startPage = (currentBlock - 1) * pagePerBlock_Log + 1;
					int endPage = startPage + pagePerBlock_Log - 1;
					...
					if (isPrev) {
						int goPrevPage = startPage - pagePerBlock_Log;			
						sb.append("&nbsp;&nbsp;<a href=\"").append("RepositorieList.jsp?pageNum="+goPrevPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						sb.append("\" title=\"<\"><</a>");
					} else {}
					...
					if (isNext) {
						int goNextPage = startPage + pagePerBlock_Log;
