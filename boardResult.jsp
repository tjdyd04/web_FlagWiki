...
<body>
	...
	<div id=Search_Top>
		...
		<form name="searchForm" action="boardResult.jsp" method="get" onsubmit="return searchCheck();" >
			<a id="link" href = "boardList.jsp" >FlagWiki</a>&nbsp;&nbsp;&nbsp;	
...
<div id=Search_Middle>
		...
		<tbody>
			...
			%>
			<tr>
				<td id="line" align="center">번호 :&nbsp; <%=totalCount - i + 1 - (pageNumTemp - 1) * listCount %></td>
				<td id="line" ><a href="boardView.jsp?num=<%=rs.getInt("NUM")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("SUBJECT") %></a></td>
				<td id="line" align="center">ID :&nbsp;<%=rs.getString("WRITER") %></td>
				<td id="line" align="center">등록일시 :&nbsp;<%=rs.getString("REG_DATE").substring(0, 10) %></td>
				<td id="line" align="center">조회수 :&nbsp;<%=rs.getInt("HIT") %></td>
			</tr>
			<%
				}
			}
